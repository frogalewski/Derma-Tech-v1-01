...
const FormulaCard: React.FC<FormulaCardProps> = ({ formula, onSave, isSaved, doctorName, iconDataUrl, isGeneratingIcon, onExpand }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden transform hover:scale-[1.02] transition-transform duration-300 border border-gray-200 dark:border-gray-700 flex flex-col">
      <div className="p-6 flex-grow">
        <div className="flex items-start space-x-4">
            <div className="flex-shrink-0">
                <div className="h-12 w-12 flex items-center justify-center bg-indigo-100 dark:bg-indigo-900/50 rounded-full">
                    {isGeneratingIcon ? (
                        ...
                        <img src={iconDataUrl} alt={`Ícone para ${formula.name}`} className="h-8 w-8 object-contain" />
                    ) : (
            ...
            <div className="flex-1">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{formula.name}</h3>
                <div className="mt-4">
                    <h4 className="text-sm font-medium text-gray-600 dark:text-gray-300">Ingredientes:</h4>
                    <ul className="mt-2 list-disc list-inside space-y-2 text-gray-700 dark:text-gray-400">
                        {formula.ingredients.map((ingredient, index) => (
                          <li key={index} className="flex justify-between items-center group">
                            <span>{ingredient}</span>
                             <button onClick={() => handleCopy(ingredient)} title="Copiar ingrediente" className="ml-2 p-1 rounded-md opacity-0 group-hover:opacity-100 focus:opacity-100 text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-opacity">
                                {copiedText === ingredient ? <CheckIcon className="h-4 w-4 text-green-500"/> : <CopyIcon className="h-4 w-4"/>}
                            </button>
                          </li>
                        ))}
                    </ul>
                ...
                <div className="mt-4">
                    <div className="flex justify-between items-center">
                        <h4 className="text-sm font-medium text-gray-600 dark:text-gray-300">Instruções de Uso:</h4>
                        <button onClick={() => handleCopy(formula.instructions)} title="Copiar instruções" className="p-1 rounded-md text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                             {copiedText === formula.instructions ? <CheckIcon className="h-4 w-4 text-green-500"/> : <CopyIcon className="h-4 w-4"/>}
                        </button>
                    </div>
                    <p className="mt-2 text-gray-700 dark:text-gray-400">{formula.instructions}</p>
                </div>
            </div>
        </div>
      </div>
      <div className="p-4 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-200 dark:border-gray-700 grid grid-cols-2 gap-2">
        <button
            ...
            aria-label={isSaved ? `Remover ${formula.name} dos salvos` : `Salvar fórmula ${formula.name}`}
        >
        ...
        <button
            ...
            aria-label={`Solicitar orçamento para ${formula.name} no WhatsApp`}
        >
        ...
        <button
            ...
            aria-label={`Exportar fórmula ${formula.name} para TXT`}
        >
         ...
         <button
            ...
            className="w-full flex items-center justify-center space-x-2 px-3 py-2 text-sm text-gray-700 dark:text-gray-300 font-medium bg-gray-200 dark:bg-gray-700 rounded-md hover:bg-gray-300 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-50 dark:focus:ring-offset-gray-800 focus:ring-indigo-500 transition-colors duration-200"
            aria-label={`Ampliar visualização da fórmula ${formula.name}`}
        >
